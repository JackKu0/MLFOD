...
class AvodModel(model.DetectionModel):
    ...
    # Other box representations  其他框表示
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        # Sets model configs (_config) 设置模型配置
    ...
    def build(self):
        ...
        # Share the same prediction dict as RPN 与RPN共享相同的预测字典
        ...
        with tf.variable_scope('avod_projection'):
            ...
            if self._config.expand_proposals_xz > 0.0:
                ...
                # Expand anchors along x and z  沿着x和z展开锚点
            ...
            with tf.variable_scope('bev'):
                # Project top anchors into bev and image spaces 项目顶部锚入bev和图像空间
                bev_proposal_boxes, bev_proposal_boxes_norm = \
                    anchor_projector.project_to_bev(
                        avod_projection_in,
                        self.dataset.kitti_utils.bev_extents)

                # Reorder projected boxes into [y1, x1, y2, x2] 将投影框重新排序为[y1, x1, y2, x2]
            ...
            with tf.variable_scope('img'):
                ...
                img_proposal_boxes, img_proposal_boxes_norm = \
                    anchor_projector.tf_project_to_image_space(
                        ...
                        image_shape)
                # Only reorder the normalized img 只对规范化的img重新排序
        ...
        with tf.variable_scope('avod_roi_pooling'):
            ...
            # Do ROI Pooling on BEV 对BEV做感兴趣区域池化
            bev_rois = tf.image.crop_and_resize(
                ...
                self._proposal_roi_crop_size,
                name='bev_rois')
            # Do ROI Pooling on image  对image做感兴趣区域池化
        ...
        # Fully connected layers (Box Predictor) 全连接层(Box Predictor)
        ...
        with tf.variable_scope('avod_encode_mb_anchors'):
            ...
            if self._box_rep == 'box_3d':
                ...
                # 为每个mb样本收集相应的ground truth anchor
                ...
                # mb sample  为每个mb样本采集相应的ground truth orientation
                ...
                # 获取boxes_3d ground truth小批处理并转换为box_8c
