...
class BevSlices(bev_generator.BevGenerator):
    ...
    def __init__(self, config, kitti_utils):
        """BEV maps created using slices of the point cloud.使用点云切片创建的BEV地图
    ...
    def generate_bev(self,
        ...
        生成BEV映射字典。为点云的每一片创建一个高度图。为整个点云创建一个密度图。
